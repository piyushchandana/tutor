...
class Rewrite_Rules extends Tutor_Base {
	...
	public function add_rewrite_rules($wp_rewrite){
		...
		foreach ($dashboard_pages as $dashboard_key => $dashboard_page){
			...
			$new_rules["(dashboard)/{$dashboard_key}/?$"] ='index.php?pagename='.$wp_rewrite->preg_index(1).'&tutor_dashboard_page=' .$dashboard_key;
			...
			//regext = ([^/]*)
			$new_rules["(dashboard)/{$dashboard_key}/(.+?)/?$"] ='index.php?pagename='.$wp_rewrite->preg_index(1).'&tutor_dashboard_page=' .$dashboard_key.'&tutor_dashboard_sub_page='.$wp_rewrite->preg_index(2);
		}
